...
import 'package:englify_app/services/profile_service.dart'; // ← ADD THIS IMPORT
...
class PersonalizationViewModel extends BaseViewModel {
  ...
  final _profileService = locator<ProfileService>(); // ← ADD THIS
  ...
  Future<void> oncontinue() async {
    ...
    try {
      ...
      // 2. ── FIX: Save name to Firestore so teacher dashboard can read it ──
      await _profileService.saveProfile(name: username);

      final user = _authservice.currentuser;

      if (user != null) {
        try {
          final hasClasses = await _classroomservice.hasStudentJoinedAnyClass(
            user.uid,
          );
